...
def load_serosurveys(model_inputs_root: Path) -> pd.DataFrame:
    ...
    data['lower'] = data['lower'].str.lower().str.strip().replace('not specified', np.nan).astype(float)
    data['upper'] = data['upper'].str.lower().str.strip().replace('not specified', np.nan).astype(float)
    data['seroprev_mean'] = data['value'] / 100
    data['seroprev_lower'] = data['lower'] / 100
    data['seroprev_upper'] = data['upper'] / 100
    data['sample_size'] = data['sample_size'].str.lower().str.strip().replace(('unchecked', 'not specified'), np.nan).astype(float)
    ...
    max_start_age = 20
    min_end_age = 60
    data['study_start_age'] = data['study_start_age'].str.lower().str.strip().replace('not specified', np.nan).astype(float)
    data['study_end_age'] = data['study_end_age'].str.lower().str.strip().replace('not specified', np.nan).astype(float)
    ...
    #    Final solution: ...
    data['geo_accordance'] = data['geo_accordance'].str.lower().str.strip().replace(('unchecked', np.nan), '0').astype(int)
    ...
    data['correction_status'] == data['correction_status'].str.lower().str.strip().replace(('unchecked', np.nan), '0').astype(int)
    ## ## ## ## ## ## ## ## ## ## ## ## ## ## ##
